...
#### 6）、触发异常
这时，你可以通过访问本地的[http://127.0.0.1:6324](http://127.0.0.1:6324)，然后触发我们构建的异常，因为我本地构建了一个服务，如果你也希望构建此服务，你可以通过安装http的服务包
```
...
### <span data-type="background" style="background-color:rgb(248, 248, 248)">webpack-sentry-plugin构建</span>
这个包目前支持webpack1\2\3版本上传sourcemap，而且周下载量也达到11k。这个包有一个优势是可以在上传后删除打包后的sourcemap，可以节省后面上传打包后文件的过滤sourcemap的操作。
<span data-type="color" style="color:#F5222D">PS: 先确保没有全局安装sentry-cli包，否则会有冲突</span>
#### 1)、安装<span data-type="background" style="background-color:rgb(248, 248, 248)">webpack-sentry-plugin插件</span>
```javascript
